...
ContextStackClient.prototype._initialize = function initialize() {
  ...
  mqttClient.subscribe(ContextStack.COMMAND.GET_CURRENT_CONTEXT_RESPONSE);
...
ContextStackClient.prototype.begin = function begin(appId) {
  const mqttClient = this.mqttClient;
  mqttClient.publish(ContextStack.COMMAND.BEGIN, stringifyJSONMessage({appId}));
};

ContextStackClient.prototype.end = function end() {
  const mqttClient = this.mqttClient;
  mqttClient.publish(ContextStack.COMMAND.END);
};
...
ContextStackClient.prototype.getCurrentContext = function getCurrentContext(callback) {
  ...
  const self = this;
  mqttClient.publish(ContextStack.COMMAND.GET_CURRENT_CONTEXT_REQUEST, stringifyJSONMessage({
