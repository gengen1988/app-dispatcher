...
ContextStack.prototype._initialize = function initialize() {
  ...
  mqttClient.on('message', (topic, message, packet) => {
    switch (topic) {
    case TOPIC.BEGIN:
      console.log('[context-stack] handle context begin');
      this._begin(parseJSONMessage(message));
      break;
    case TOPIC.END:
      console.log('[context-stack] handle context end');
      this._end();
      break;
    case TOPIC.GET_CURRENT_CONTEXT_REQUEST:
      console.log('[context-stack] handle context query');
...
ContextStack.prototype._getCurrentContext = function getCurrentContext(msg) {
  ...
  redisClient.lrange('contextstack', 0, 0, function(err, appIds) {
    ...
    console.log('[context-stack] query result:', appIds);
    msg.appId = appIds[0];
